...
namespace ReciboTiendaPC.Reportes.Controllers
{
    public class ReportesController : Controller
    {
        ...
        [AuthorizationPrivilegeFilter]
        public ActionResult EliminaDetalleFolioTablaPaso(string folio)
        {
            try
            {
                Hashtable hasCat = new Hashtable();
                ngFolioMciaCad.NG_EliminaDetalleFolioTablaPaso(folio);
                hasCat["result"] = "1";
                return Json(hasCat, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
        ...
        [AuthorizationPrivilegeFilter]
        public ActionResult GuardarFolioTablaPasoAux(string folio, string codigobarra, string cantCapturarada, string lote)
        {
            try
            {
                Hashtable hasCat = new Hashtable();
        ...
        #region Reporte Acta Destrucción
        [AuthorizationPrivilegeFilter]
        public JsonResult ImprimeReporte(string folio, string Usuario)
        {
            string RelativePath = string.Empty;
            try
            {
                reporteReq ReqDatosGenerales = new reporteReq();
                ReqDatosGenerales.Id_Num_Aplic = 22;
                ReqDatosGenerales.Id_Num_Rpt = 2;
                ReqDatosGenerales.Id_Num_ActaDestruccion = Int32.Parse(folio);
                ReqDatosGenerales.impreso_Por = Usuario;

                RelativePath = RptActaDestruccion(ReqDatosGenerales);
        ...
        private string RptActaDestruccion(reporteReq ReqDatosGenerales)
        {
            try
            {
                #region config reporte
                ReportViewer viewer = new ReportViewer();
                viewer.ProcessingMode = ProcessingMode.Local;
                string path = Path.Combine(Server.MapPath("~/Reportes/rdlc"), "rpt_ActaDestruccion.rdlc");

                if (!System.IO.File.Exists(path))
                {
                    throw new Exception("Error al imprimir. Archivo no encontrado");
                }
                ...
                ngReportes.Rpt_ActaDestruccion(ReqDatosGenerales, ref viewer);
                SavePdf(viewer, SavePath);
                return RelativePath;
        ...
        #region Reporte Mermas
        [AuthorizationPrivilegeFilter]
        public JsonResult ImprimeReporteMermas(string Fecha, int Jefe, string nombre, string Usuario)
        {
            string RelativePath = string.Empty;
            try
            {
                reporteReq ReqDatosGenerales = new reporteReq();
                ReqDatosGenerales.Id_Num_Aplic = 22;
                ReqDatosGenerales.Id_Num_Rpt = 6;
                ReqDatosGenerales.Usuario = Jefe;
                ReqDatosGenerales.FechaConsulta = Fecha;
                ReqDatosGenerales.Nombre_Usuario = nombre;
                ReqDatosGenerales.impreso_Por = Usuario;

                RelativePath = RptMermas(ReqDatosGenerales);
        ...
        private string RptMermas(reporteReq ReqDatosGenerales)
        {
            try
            {
                #region config reporte
                ReportViewer viewer = new ReportViewer();
                viewer.ProcessingMode = ProcessingMode.Local;
                string path = Path.Combine(Server.MapPath("~/Reportes/rdlc"), "rpt_Merma.rdlc");
                ...
                ngReportes.Rpt_Mermas(ReqDatosGenerales, ref viewer);
                SavePdf(viewer, SavePath);
                return RelativePath;
        ...
        [AuthorizationPrivilegeFilter]
        public JsonResult GeneraMermasMasivo(string Fecha, int Jefe)
        {
            ...
            try
            {
                ...
                ds = ngReportes.NGGeneraMermasMasivo(Fecha, Jefe);
                if (ds != null)
                {
                    if (ds.Tables.Count > 0)
                    {
                        foreach (System.Data.DataRow row in ds.Tables[0].Rows)
                        {
                            ...
                            {
                                Mensaje = "El Folio de Acta de Destrucción Asignado es el: " + row[1].ToString().Trim()
                                            + " con fecha del: " + row[2].ToString().Trim();
                            }
                            else
                            {
                                Mensaje = "No se generó Acta de Destrucción.";
                            }
                        }
                    }
                }
               
                var result = new { Success = true, Message = "La generación de mermas se realizó con éxito.<br><br>" + Mensaje };
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        #region Reporte ArtProxCadEtapa
        [AuthorizationPrivilegeFilter]
        public JsonResult ImprimeReporteArtProxCadEtapa(string Fecha, int Jefe, int categoria, int tipovencimiento, string nombre_Categoria, string nombre_Vencimiento, string nombre_Jefe, string Usuario)
        {
            string RelativePath = string.Empty;
            try
            {
                reporteReq ReqDatosGenerales = new reporteReq();
                ReqDatosGenerales.Id_Num_Aplic = 22;
                ReqDatosGenerales.Id_Num_Rpt = 5;
                ReqDatosGenerales.Usuario = Jefe;
                ReqDatosGenerales.FechaConsulta = Fecha;
                ReqDatosGenerales.pId_Num_Categ = categoria;
                ReqDatosGenerales.pIdOpc = tipovencimiento;
                ReqDatosGenerales.Nombre_Categoria = nombre_Categoria;
                ReqDatosGenerales.Nombre_Vencimiento = nombre_Vencimiento;
                ReqDatosGenerales.Nombre_Usuario = nombre_Jefe;
                ReqDatosGenerales.impreso_Por = Usuario;

                RelativePath = RptArtProxCadEtapa(ReqDatosGenerales);
        ...
        private string RptArtProxCadEtapa(reporteReq ReqDatosGenerales)
        {
            try
            {
                #region config reporte
                ReportViewer viewer = new ReportViewer();
                viewer.ProcessingMode = ProcessingMode.Local;
                string path = Path.Combine(Server.MapPath("~/Reportes/rdlc"), "rpt_ArticulosRematadosProximosCaducarXEtapa.rdlc");
                ...
                ngReportes.Rpt_ArtProxCadEtapa(ReqDatosGenerales, ref viewer);
                SavePdf(viewer, SavePath);
                return RelativePath;
        ...
        #region Reporte FolioMerCadCap
        [AuthorizationPrivilegeFilter]
        public JsonResult ImprimeReporteFolioMerCadCap(string folio, string Fecha, string jefe, string Usuario)
        {
            string RelativePath = string.Empty;
            try
            {
                reporteReq ReqDatosGenerales = new reporteReq();
                ReqDatosGenerales.Id_Num_Aplic = 22;
                ReqDatosGenerales.Id_Num_Rpt = 4;
                ReqDatosGenerales.Id_Num_ActaDestruccion = Int32.Parse(folio);
                ReqDatosGenerales.Nombre_Usuario = jefe;
                ReqDatosGenerales.FechaConsulta = Fecha;
                ReqDatosGenerales.impreso_Por = Usuario;

                RelativePath = RptFolioMerCadCap(ReqDatosGenerales);
        ...
        private string RptFolioMerCadCap(reporteReq ReqDatosGenerales)
        {
            try
            {
                #region config reporte
                ReportViewer viewer = new ReportViewer();
                viewer.ProcessingMode = ProcessingMode.Local;
                string path = Path.Combine(Server.MapPath("~/Reportes/rdlc"), "rpt_MercanciaProximaCaducarAutorizada.rdlc");
                ...
                ngReportes.Rpt_FolioMerCadCap(ReqDatosGenerales, ref viewer);
                SavePdf(viewer, SavePath);
                return RelativePath;
        ...
        [AuthorizationPrivilegeFilter]
        public JsonResult ImprimeReporteCapturaMercancíaCaducar(string folio, string Usuario)
        {
            string RelativePath = string.Empty;
            try
            {
                reporteReq ReqDatosGenerales = new reporteReq();
                ReqDatosGenerales.Id_Num_Aplic = 22;
                ReqDatosGenerales.Id_Num_Rpt = 1;
                ReqDatosGenerales.Id_Num_ActaDestruccion = Int32.Parse(folio);
                ReqDatosGenerales.impreso_Por = Usuario;

                RelativePath = RptCapturaMercancíaCaducar(ReqDatosGenerales);
        ...
        private string RptCapturaMercancíaCaducar(reporteReq ReqDatosGenerales)
        {
            try
            {
                #region config reporte
                ReportViewer viewer = new ReportViewer();
                viewer.ProcessingMode = ProcessingMode.Local;
                string path = Path.Combine(Server.MapPath("~/Reportes/rdlc"), "rpt_CapturaMercanciaPorCaducar.rdlc");
                ...
                ngReportes.Rpt_CapturaMercancíaCaducar(ReqDatosGenerales, ref viewer);
                SavePdf(viewer, SavePath);
                return RelativePath;
        ...
        #region Reporte SolucionesAsignadas
        [AuthorizationPrivilegeFilter]
        public JsonResult ImprimeReporteSolucionesAsignadas(string FechaInicial, string FechaFinal, string Usuario)
        {
            string RelativePath = string.Empty;
            try
            {
                reporteReq ReqDatosGenerales = new reporteReq();
                ReqDatosGenerales.Id_Num_Aplic = 22;
                ReqDatosGenerales.Id_Num_Rpt = 7;
                ReqDatosGenerales.FechaConsulta = FechaInicial;
                ReqDatosGenerales.Nombre_Usuario = FechaFinal;
                ReqDatosGenerales.bit = false;
                ReqDatosGenerales.impreso_Por = Usuario;

                RelativePath = RptSolucionesAsignadas(ReqDatosGenerales);
        ...
        private string RptSolucionesAsignadas(reporteReq ReqDatosGenerales)
        {
            try
            {
                #region config reporte
                ReportViewer viewer = new ReportViewer();
                viewer.ProcessingMode = ProcessingMode.Local;
                string path = Path.Combine(Server.MapPath("~/Reportes/rdlc"), "rpt_SolucionesAsignadas.rdlc");
                ...
                ngReportes.Rpt_SolucionesAsignadas(ReqDatosGenerales, ref viewer);
                SavePdf(viewer, SavePath);
                return RelativePath;
