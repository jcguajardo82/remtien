var iresult = 4;
var tblpaso = 0;
$(function () {

    var a = 0;
    //$("#allBox").click(function () {
    //    $(".box").attr("checked", $(this).attr("checked") ? true : false);
    //});
    //Cuando se ha aceptado el folio

    $(".btnfolioaceptado").click(function (event) {
        RefrescarPagina();
    });
    $(".btnfoliocancelado").click(function (event) {
        RefrescarPagina();
    });
    //Evento del Boton de buscar folio para el reporte
    $(".btn-buscarfolmciacadacep").click(function (event) {
        var isAllValid = true;
        isAllValid = validarRepDatosEntrada();
        if (isAllValid) {
            BuscarFolioMciaCadReporte();
        }

    });
    //Evento del Boton de buscar folio buscar folio para la autorizacion
    $(".btn-buscarfolmciacadacepAut").click(function (event) {
        var isAllValid = true;

        if ($('#Id_Num_Dpto').val() == "-1") {
            isAllValid = false;
            var $div = $('#Id_Num_Dpto').parent();
            var $span = $div.children()[2];
            $span.setAttribute("style", "visibility:visible");

        }
        else {
            var $div = $('#Id_Num_Dpto').parent();
            var $span = $div.children()[2];
            $span.setAttribute("style", "visibility:hidden");

        }
        if ($('#FechaFolioAut').val() == "") {
            isAllValid = false;
            var $div = $('#FechaFolioAutDate').parent();
            var $span = $div.children()[2];
            $span.setAttribute("style", "visibility:visible");

        }
        else {
            var $div = $('#FechaFolioAutDate').parent();
            var $span = $div.children()[2];
            $span.setAttribute("style", "visibility:hidden");

        }
        if ($('#FolCap').val() == "-1") {
            isAllValid = false;
            //var $div = $('#FolCap').parent();
            //var $span = $div.children()[2];
            //$span.setAttribute("style", "visibility:visible");
            var $div = $('#FolCap').parent();
            var $span = $div.children()[2];
            $span.setAttribute("style", "visibility:visible");


        }
        else {
            var $div = $('#FolCap').parent();
            var $span = $div.children()[2];
            $span.setAttribute("style", "visibility:hidden");
            //var $div = $('#FolCap').parent();
            //var $span = $div.children()[2];
            //$span.setAttribute("style", "visibility:hidden");

        }
        if (isAllValid) {
            BuscarFolioMciaCadAutorizar();
        }

    });
    //Evento para autorizar folios
    $(".btnAutFolio").click(function (event) {
        var isAllValid = true;
        var isValCant = 3;
        isAllValid = validarDatosEntrada();
        ValidaCantidadesAutorizar();
        isValCant = iresult;
        tblpaso = 0;
        if (isAllValid && isValCant == 3) {
            GuardarFolioTablaPaso();

        }
        else {
            if (isValCant == 2) {
                $('#modalCancelarFolio').modal();
            }
            else {
                if (isValCant == 0) {
                    alert('La cantidad confirmada no puede ser menor que 0 o vacío.');
                }

            }


            //var isAllValid = true;
            //var isValCant = 3;
            //isAllValid = validarDatosEntrada();
            //ValidaCantidadesAutorizar();
            //isValCant = iresult;
            //tblpaso = 0;
            //if (isAllValid && isValCant == 3) {
            //    GuardarFolioTablaPaso();
            //    if (tblpaso == 1) {
            //        AutorizarFolios();
            //    }
            //}
            //else {
            //    if (isValCant == 2) {
            //        $('#modalCancelarFolio').modal();
            //    }
            //    else {
            //        if (isValCant == 0) {
            //            alert('La cantidad confirmada no puede ser menor que 0 o vacío.');
            //        }

            //    }
            //}
        }
    });
    //Evento para cancelar folios
    $(".btnCancelarFolio").click(function (event) {
        var isAllValid = true;
        isAllValid = validarDatosEntrada();
        if (isAllValid) {
            CancelarFolios();
        }

    });
    $(".btnCanFolio").click(function (event) {
        CancelarFolios();
        $("body").css({ "padding-right": "0px" });
    });

    //$('#FechaFolioDate input').datepicker({
    //    autoclose: true,
    //    todayHighlight: true,
    //    onSelect: function (date) {
    //        alert(date);
    //    }
    //});
    $('#FechaFolioAutDate input').datepicker({
        autoclose: true,
        todayHighlight: true,
        clearBtn: true,
        format: 'yyyy-mm-dd' //,
    }).on('changeDate', function (value, date) {
        LlenarListFolioAutorizar();
    });
    $("#Id_Num_Dpto").change(function () {
        var ddlNumDepto = $('#Id_Num_Dpto').val();
        if (ddlNumDepto != "-") {
            LlenarListFolioAutorizar();
        }
    });

    $('#RepFechaFolioDate input').datepicker({
        autoclose: true,
        todayHighlight: true,
        clearBtn: true,
        format: 'yyyy-mm-dd',
    }).on('changeDate', function (value, date) {
        LlenarListRepFolio();

    });
});
function LlenarListFolioAutorizar() {
    var fecha = $("#FechaFolioAut").val();
    var jefedepto = $("#Id_Num_Dpto").val();
    ddlFolCap = $('#FolCap');

    $.ajax({
        type: 'GET',
        cache: false,
        url: '../Reportes/iniciarComboFolioAutorizar',
        data: { jefedepto: jefedepto, fecha: fecha },
        success: function (data) {
            ddlFolCap.empty();
            ddlFolCap.append($('<option></option>').val('-1').html('Seleccione'));
            for (var i = 0; i < data.length; i++) {
                var x = data[i].Value;
                var y = data[i].Text;
                ddlFolCap.append($('<option></option>').val(x).html(y));
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert("Ocurrio un error al cargar (Listado Folio): " + errorThrown);
        }
    });

}

function LlenarListRepFolio() {
    var fecha = $("#RepFechaFolio").val();
    var jefedepto = $("#RepIdDpto").val();
    ddlRepFolCap = $('#RepFolCap');

    $.ajax({
        type: 'GET',
        cache: false,
        url: '../Reportes/iniciarComboRepFolio',
        data: { jefedepto: jefedepto, fecha: fecha },
        success: function (data) {
            ddlRepFolCap.empty();
            ddlRepFolCap.append($('<option></option>').val('-1').html('Seleccione'));
            for (var i = 0; i < data.length; i++) {
                var x = data[i].Value;
                var y = data[i].Text;
                ddlRepFolCap.append($('<option></option>').val(x).html(y));
            }
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert("Ocurrio un error al cargar (Listado Folio): " + thrownError);
        }
    });

}

function BuscarFolioMciaCadReporte() {
    var FolCap = $("#RepFolCap").val();
    var Fecha = $("#RepFechaFolio").val();

    $.ajax({
        type: 'GET',
        cache: false,
        url: '../Reportes/ObtenerGridRepFolioMerCadCapPartial',
        data: { folio: FolCap, fecha: Fecha },
        success: function (data) {
            $('#divResults').html(data);
            $('#Reporte').show();
            //pintartablaReporte();
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
        }

    });


    //$.ajax({
    //    cache: false,
    //    type: "POST",
    //    url: '../Reportes/ObtenerGridFolioMerCadCap',
    //    async: true,
    //    dataType: "json",
    //    data: { jefedepto: Id_Num_Dpto, fecha: FechaFolio, folio: FolCap },
    //    success: function (d) {
    //        $("#tblbodyFolioMciaCad").html("");
    //        if (d && d.length > 0) {
    //            $("#tblbodyFolioMciaCad").html(d);
    //        }
    //    },
    //    error: function (xhr, ajaxOptions, thrownError) {
    //        alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
    //    }
    //});
}
function BuscarFolioMciaCadAutorizar() {
    var Id_Num_Dpto = $("#Id_Num_Dpto").val();
    var FechaFolio = $("#FechaFolioAut").val();
    var FolCap = $("#FolCap").val();
    $.ajax({
        type: 'GET',
        cache: false,
        url: '../Reportes/ObtenerGridFolioMerCadCapPartial',
        data: { jefedepto: Id_Num_Dpto, fecha: FechaFolio, folio: FolCap },
        success: function (data) {
            $('#divResults').html(data);
            pintartabla();
        },
        error: function (jqXHR, textStatus, errorThrown) {
            alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
        }

    });
}

function pintartabla() {

    $('.gvv').DataTable({
        dom: '<"html5buttons"B>lTfgitp',
        "pageLength": 50,
        buttons: [
            { extend: 'pdf', title: 'Folio Mercancia Caduca Capturada', className: 'btn-danger' }
        ]
    });
    $('.dataTables_filter').addClass('text-right');
    $('.dataTables_paginate').addClass('text-right');
    $('.html5buttons').addClass('btn-danger');
    $('.buttons-html5').addClass('btn-danger');

}

function pintartablaReporte() {

    $('.gvv').DataTable({
        dom: '<"html5buttons"B>lTfgitp',
        "pageLength": 50,
        buttons: [
            { extend: 'pdf', title: 'Reporte de Mercancia Próxima a Caducar Autorizada', className: 'btn-danger', orientation: 'landscape', pageSize: 'LEGAL' }
        ]
    });
    $('.dataTables_filter').addClass('text-right');
    $('.dataTables_paginate').addClass('text-right');
    $('.html5buttons').addClass('btn-danger');
    $('.buttons-html5').addClass('btn-danger');

}

function pintartablaAutorizados() {
    //$(".gvv").prepend($("<thead></thead>").append($(this).find("tr:first"))).dataTable();
    //$('.dataTables_filter').addClass('text-right');
    //$('.dataTables_paginate').addClass('text-right');

    //$('.gvv').DataTable({
    //    dom: '<"html5buttons"B>lTfgitp',
    //    "pageLength": 50,
    //    buttons: [
    //        { extend: 'pdf', title: 'Folio Mercancia Caduca Capturada' }
    //    ]
    //});
    //$('.dataTables_filter').addClass('text-right');
    //$('.dataTables_paginate').addClass('text-right');

    //$("#divResults").prepend($("<thead></thead>").append($(this).find("tr:first"))).dataTable();
    //$('.dataTables_filter').addClass('text-right');
    //$('.dataTables_paginate').addClass('text-right');

}
function AutorizarFolios() {
    var Id_Num_Dpto = $("#Id_Num_Dpto").val();
    var FechaFolio = $("#FechaFolioAut").val();
    var FolCap = $("#FolCap").val();

    $.ajax({
        cache: false,
        type: "POST",
        url: '../Reportes/AutorizarFolios',
        async: true,
        dataType: "json",
        data: { folio: FolCap },
        success: function (d) {
            $("#divRenderAprobar").html("");
            $("#divRenderAprobar").append(d.result);
            $('#modalAcepFolio').modal();
            //if (d.result == "1") {
            //    $("#divRenderAprobar").html("");
            //    $("#divRenderAprobar").append("El Folio ha sido aceptado con Éxito");
            //    $('#modalAcepFolio').modal();
            //}
            //else {
            //    $("#divRenderAprobar").html("");
            //    $("#divRenderAprobar").append(d.result);
            //    $('#modalAcepFolio').modal();
            //    //alert(d.result);
            //}

        },
        error: function (xhr, ajaxOptions, thrownError) {
            alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
        }
    });
}

function CancelarFolios() {
    var Id_Num_Dpto = $("#Id_Num_Dpto").val();
    var FechaFolio = $("#FechaFolioAut").val();
    var FolCap = $("#FolCap").val();

    $.ajax({
        cache: false,
        type: "POST",
        url: '../Reportes/CancelarFolios',
        async: true,
        dataType: "json",
        data: { folio: FolCap },
        success: function (d) {
            // $('#modalCancelFolio').modal();
            //AvisoCancelado();
            $("#divRenderCancelar").html("");
            $("#divRenderCancelar").append(d.result);
            $('#modalCancelFolio').modal();
        },
        error: function (xhr, ajaxOptions, thrownError) {
            alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
        }
    });
}
function AvisoCancelado() {
    $('#modalCancelFolio').modal('show');
}
function validarDatosEntrada() {
    var isAllValid = true;

    if ($('#Id_Num_Dpto').val() == "-1") {
        isAllValid = false;
        var $div = $('#Id_Num_Dpto').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:visible");

    }
    else {
        var $div = $('#Id_Num_Dpto').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:hidden");

    }
    if ($('#FechaFolioAut').val() == "") {
        isAllValid = false;
        var $div = $('#FechaFolioAutDate').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:visible");

    }
    else {
        var $div = $('#FechaFolioAutDate').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:hidden");

    }

    if ($('#FolCap').val() == "-1") {
        isAllValid = false;
        //var $div = $('#FolCap').parent();
        //var $span = $div.children()[2];
        //$span.setAttribute("style", "visibility:visible");
        var $div = $('#FolCap').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:visible");


    }
    else {
        var $div = $('#FolCap').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:hidden");
        //var $div = $('#FolCap').parent();
        //var $span = $div.children()[2];
        //$span.setAttribute("style", "visibility:hidden");

    }
    return isAllValid;
}

function validarRepDatosEntrada() {
    var isAllValid = true;

    if ($('#RepIdDpto').val() == "-1") {
        isAllValid = false;
        var $div = $('#RepIdDpto').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:visible");

    }
    else {
        var $div = $('#RepIdDpto').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:hidden");

    }
    //
    if ($('#RepFechaFolio').val() == "") {
        isAllValid = false;
        var $div = $('#RepFechaFolioDate').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:visible");

    }
    else {
        var $div = $('#RepFechaFolioDate').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:hidden");

    }

    if ($('#RepFolCap').val() == "-1") {
        isAllValid = false;
        //var $div = $('#FolCap').parent();
        //var $span = $div.children()[2];
        //$span.setAttribute("style", "visibility:visible");
        var $div = $('#RepFolCap').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:visible");


    }
    else {
        var $div = $('#RepFolCap').parent();
        var $span = $div.children()[2];
        $span.setAttribute("style", "visibility:hidden");
        //var $div = $('#FolCap').parent();
        //var $span = $div.children()[2];
        //$span.setAttribute("style", "visibility:hidden");

    }

    return isAllValid;
}
function ValidaCantidadesAutorizar() {
    var ArrayCate = [];
    var countrowchk = $('.gvv').find('tr').length;
    var ichk = 0;
    iresult = 3;

    $('.gvv').find('tr').each(function () {
        var row = $(this);
        $text = row.find('input[type="text"]');
        $text.attr('style', 'background-color:#ffffff');
    });

    //Validar que si dio check en almenos uno ini
    if (iresult == 3) {
        $('.gvv').find('tr').each(function () {
            var row = $(this);
            var cantcap = row.find('input[type="text"]').val();
            if ((row.find('input[type="checkbox"]').is(':checked') && parseInt(cantcap) < 0) || (row.find('input[type="checkbox"]').is(':checked') && cantcap == "") || (row.find('input[type="checkbox"]').is(':checked') && cantcap == undefined)) {
                $text = row.find('input[type="text"]')
                $text.attr('style', 'background-color:#f11');
                //alert('La cantidad confirmada no puede ser menor que 0 o vacío.');
                //$('#modalCancelarFolio').modal();
                //return false;     
                iresult = 0;//Dio Check, pero no puso valor            
                //return false;
            }
        });
    }

    //fin

    //Validar que si check al menos uno, que el valor no sea mayor. ini
    if (iresult == 3) {
        $('.gvv').find('tr').each(function () {
            var row = $(this);
            var cantcap = row.find('input[type="text"]').val();
            if (row.find('input[type="checkbox"]').is(':checked') && parseInt(cantcap) > 0 && row.find('input[type="text"]').val() != "") {
                var cantCapturarada = row.find('input[type="text"]').val();
                $text = row.find('input[type="text"]')

                $td = row.find('td')[4];
                var cantSugerida = $td.innerText;
                if (parseInt(cantCapturarada) > parseInt(cantSugerida)) {
                    alert('La cantidad confirmada no puede ser mayor a la cantidad sugerida.');
                    $text.attr('style', 'background-color:#f11');
                    //return false;
                    iresult = 1;//Dio Check, pero  puso valor mayor que la cantidaad sugerida
                    //return false;
                }
                else {
                    $text.attr('style', 'background-color:#ffffff');
                }
            }
        });
    }

    //fin

    //Validar que no check en nada y cancelar. ini
    if (iresult == 3) {
        $('.gvv').find('tr').each(function () {
            var row = $(this);
            var cantcap = row.find('input[type="text"]').val();
            var chkunchk = row.find('input[type="checkbox"]').is(':checked');

            if (chkunchk == false) {
                $text = row.find('input[type="text"]')
                //$text.attr('style', 'background-color:#f11');
                $text.attr('style', 'background-color:#ffffff');
                ichk += 1;
            } else {
                $text = row.find('input[type="text"]')
                $text.attr('style', 'background-color:#ffffff');
            }
        });

        if (ichk == countrowchk) {
            //$('#modalCancelarFolio').modal();
            iresult = 2;
            //return false;
        }
    }


    //fin

    //$('.gvv').find('tr').each(function () {
    //    var row = $(this);
    //    var cantcap = row.find('input[type="text"]').val();
    //    if ((row.find('input[type="checkbox"]').is(':checked') && parseInt(cantcap) < 0) || (row.find('input[type="checkbox"]').is(':checked') && cantcap == "") || (row.find('input[type="checkbox"]').is(':checked') && cantcap == undefined)) {
    //        $text = row.find('input[type="text"]')
    //        $text.attr('style', 'background-color:#f11');
    //        //$('#modalCancelarFolio').modal();
    //        isAllValid = false;
    //    }
    //    else {
    //        if ((parseInt(cantcap) < 0) || (cantcap == "") || (cantcap == undefined)) {
    //            $text = row.find('input[type="text"]')
    //            $text.attr('style', 'background-color:#f11');
    //            //$('#modalCancelarFolio').modal();
    //            ichk += 1;
    //            isAllValid = false;
    //        } else {
    //            $text = row.find('input[type="text"]')
    //            $text.attr('style', 'background-color:#ffffff');
    //        }

    //    }
    //    //if ((row.find('input[type="checkbox"]').is(':checked') && parseInt(cantcap) > 0) || (row.find('input[type="checkbox"]').is(':checked') && row.find('input[type="text"]').val() != "")) {

    //    if (row.find('input[type="checkbox"]').is(':checked') && parseInt(cantcap) > 0 && row.find('input[type="text"]').val() != "") {
    //        var cantCapturarada = row.find('input[type="text"]').val();
    //        $text = row.find('input[type="text"]')

    //        $td = row.find('td')[4];
    //        var cantSugerida = $td.innerText;
    //        if (parseInt(cantCapturarada) > parseInt(cantSugerida)) {
    //            alert('La cantidad confirmada no puede ser mayor a la cantidad sugerida.');
    //            $text.attr('style', 'background-color:#f11');
    //            isAllValid = false;
    //            return false;
    //        }
    //        else {
    //            $text.attr('style', 'background-color:#ffffff');
    //            isAllValid = true;
    //        }
    //    }
    //});

    //if (!isAllValid) {
    //    alert("La cantidad confirmada no puede ser mayor a la cantidad sugerida.");
    //}

    //$('.gvv tr').each(function (i, row) {
    //    var $actualrow = $(row);
    //    $checkbox = $actualrow.find(':checkbox:checked');
    //    console.log($checkbox);
    //});
}
function boxDisable(e) {
    var a = 0;
}
//function agregarSearch() {


// DataTable
//var table = $('.gvv').DataTable();
/*
// Apply the search
table.columns().eq(0).each(function (colIdx) {
    $('input', table.column(colIdx).header()).on('keyup change', function () {
        table
            .column(colIdx)
            .search(this.value)
            .draw();
    });
});

$('.dataTables_filter').addClass('text-right');
$('.dataTables_paginate').addClass('text-right');

*/


//$('.gvv').DataTable({
//    dom: '<"html5buttons"B>lTfgitp',
//    "pageLength": 50,
//    buttons: [
//        { extend: 'pdf', title: 'Folio Mercancia Caduca Capturada' }
//    ]
//});
//$('.dataTables_filter').addClass('text-right');
//$('.dataTables_paginate').addClass('text-right');

//$("#divResults").prepend($("<thead></thead>").append($(this).find("tr:first"))).dataTable();
//$('.dataTables_filter').addClass('text-right');
//$('.dataTables_paginate').addClass('text-right');
/*
var table = $('#divResults').DataTable();

$('#divResults tfoot th').each(function () {
    var title = $(this).text();
    $(this).html('<input type="text" placeholder="Search ' + title + '" />');
});

// DataTable
var table = $('#example').DataTable();

// Apply the search
table.columns().every(function () {
    var that = this;

    $('input', this.footer()).on('keyup change', function () {
        if (that.search() !== this.value) {
            that
                .search(this.value)
                .draw();
        }
    });
});
*/
//}

//puedes ponerlo en el init
//pintartablaAutorizados();

//$(".btnAutFolio").click(function (event) {
//    BuscarFolioMciaCadAutorizar();
//});

//$('.gvv').DataTable({
//   dom: '<"html5buttons"B>lTfgitp',
//    //"pageLength": 50,
//    buttons: [
//        { extend: 'pdf', title: 'Catalogo Almacen' }
//    ]
//});
//$('.dataTables_filter').addClass('text-right');
//$('.dataTables_paginate').addClass('text-right');

//Con esto agregas un ID a la tabla
//$('#divResults').find('table.gvv').first().attr('id', 'treporte');

//Con esta sentencia agregas imagenes al reporte.


//$('#treporte').DataTable({
//    dom: '<"html5buttons"B>lTfgitp',
//    buttons: [
//        {
//            extend: 'pdfHtml5',
//            customize: function (doc) {
//                doc.content.splice(1, 0, {
//                    margin: [0, 0, 0, 12],
//                    alignment: 'center',
//                    image: 'data:image/png;base64,'
//                });
//            }
//        }
//    ]
//});
//$('.dataTables_filter').addClass('text-right');
//$('.dataTables_paginate').addClass('text-right');
function GuardarFolioTablaPaso() {
    var countchk = $('input:checkbox:checked').length;
    var FolCap = $("#FolCap").val();
    debugger;
    //borra los registros para el folio en la tabla de paso
    $.ajax({
        cache: false,
        type: "get",
        url: '../Reportes/EliminaDetalleFolioTablaPaso',
        async: true,
        dataType: "json",
        data: { folio: FolCap },
        success: function (d) {
            if (d.Success != false) {
                $('.gvv').find('tr').each(function () {
                    var row = $(this);
                    if ((row.find('input[type="checkbox"]').is(':checked'))) {

                        //var FolCap = $("#FolCap").val();
                        var cantCapturarada = row.find('input[type="text"]').val();
                        var $codigobarra_aux = row.find('td')[0];
                        var $codigobarra_aux_2 = $codigobarra_aux.childNodes;
                        var codigobarra = $codigobarra_aux_2[0].textContent;

                        var $lote_aux = row.find('td')[2];
                        var $lote_aux_2 = $lote_aux.childNodes;
                        var lote = $lote_aux_2[0].textContent;



                        //////////////------------------------
                        $.ajax({
                            cache: false,
                            type: "get",
                            url: '../Reportes/GuardarFolioTablaPasoAux',
                            async: true,
                            dataType: "json",
                            data: { folio: FolCap, codigobarra: codigobarra, cantCapturarada: cantCapturarada, lote: lote },
                            success: function (d) {
                                if (d.Success != false) {
                                    tblpaso += 1;
                                    if (tblpaso == countchk) {
                                        AutorizarFolios();
                                    }
                                }
                                else { alert(d.Message); }
                            },
                            error: function (xhr, ajaxOptions, thrownError) {
                                debugger
                                alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
                            }
                        });
                    }
                });

            }
            else { alert(d.Message); }
        },
        error: function (xhr, ajaxOptions, thrownError) {
            debugger
            alert("Ocurrio un error al cargar (Folio Mercancía Caduca): " + thrownError);
        }
    });
   
}

function RefrescarPagina() {
    window.location.reload();
}